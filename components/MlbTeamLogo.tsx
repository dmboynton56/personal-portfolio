...
// react-mlb-logos bundles every team SVG. Loading it lazily keeps it out of
// the page's first-load JS; the placeholder renders until it arrives.
const LazyMlbLogo = dynamic(
  () =>
    import('react-mlb-logos').then((mod) => {
...
export function MlbTeamLogo({ team, size = 32, className }: Props) {
  ...
  return (
    ...
    >
      <LazyMlbLogo code={meta.code} size={size} />
    </div>
  )
}
