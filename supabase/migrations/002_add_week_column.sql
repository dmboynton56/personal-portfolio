...
comment on column games.book_spread is 'Consensus spread (home team perspective).';

-- Refresh dependent view so callers can access the new column
drop view if exists games_today_enriched;

create view games_today_enriched as
select
  g.id as game_id,
  g.league,
  g.season,
